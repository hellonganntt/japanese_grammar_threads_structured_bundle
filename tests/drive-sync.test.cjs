...
(async () => {
  ...
  await db.reconcileCatalog({
    version: "drive-test",
    cards: [{ id: "l36-v001", lesson: 36, lessonIndex: 0, order: 0 }]
  });
  await db.rateCard("l36-v001", "good", new Date("2026-06-24T10:00:00.000Z"));

  const exported = await db.exportSnapshot("device-a");
  assert.equal(exported.schemaVersion, 3);
  assert.equal(exported.deviceId, "device-a");
  assert.deepEqual(Object.keys(exported.cards), ["l36-v001"]);
  ...
  assert.equal((await db.getCard("l36-v001")).intervalDays, 20);
  ...
  await db.rateCard("l36-v001", "hard", new Date("2026-06-26T10:00:00.000Z"));
