(function(root, factory){
  ...
  class Database {
    ...
    async reconcileCatalog(catalog){
      ...
      if(currentVersion === catalog.version) return false;
        ...
        await cursorEach(cards.openCursor(), cursor => {
          ...
          if(entry){
            ...
            if(
              ...
              existing.lessonIndex !== lessonIndex ||
              existing.order !== order ||
              existing.available === false
            ){
              cursor.update({ ...existing, lesson, lessonIndex, order, available: true });
            }
            incoming.delete(existing.id);
          }else if(existing.available !== false){
            cursor.update({ ...existing, available: false });
          }
        });

        incoming.forEach(entry => cards.put(SRSCore.createCatalogCard(entry)));
        tx.objectStore(SETTINGS_STORE).put({ key: CATALOG_VERSION_KEY, value: catalog.version });
        await done;
        return true;
      });
    }
    ...
    async migrateLegacy(rawValue){
      ...
      const snapshot = SRSCore.normalizeSnapshot(parsed);
    ...
    async getStats(now = new Date(), newLimit = 10){
      await this.open();
    ...
    async buildDailyQueue(now = new Date(), newLimit = 10, extraNewLimit = 0){
      await this.open();
      const stats = await this.getStats(now, newLimit);
      const tx = this.db.transaction(CARDS_STORE, "readonly");
      const done = transactionDone(tx);
      const store = tx.objectStore(CARDS_STORE);
      const due = [];
      ...
      await cursorEach(store.index("order").openCursor(), cursor => {
        const card = cursor.value;
        if(card.available !== false && card.state === "new") unseen.push({ ...card, queueType: "new" });
      });
      await done;
    ...
    async mergeSnapshot(value){
      const snapshot = SRSCore.normalizeSnapshot(value);
      if(!snapshot) throw new Error("The Google Drive progress file is invalid or uses an unsupported version.");
      if(Object.keys(snapshot.cards).length !== Object.keys(value.cards).length){
